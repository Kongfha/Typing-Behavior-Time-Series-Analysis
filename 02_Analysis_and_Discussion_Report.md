...
| RQ4 | Recovery style as a behavioral signature | **Answered (exploratory)** | Heuristic recovery summaries, DTW grouping, and per-trial `error_suffix_len_after` MP suggest descriptive recovery-pattern groupings such as *deep-overshoot-then-recovery*, *pause-then-delete*, *immediate-repair*, and *shallow*. These should be read as dominant tendencies, not sharply separated stable latent classes. |
...
**Observation — mean duration is not the same as error rate.** U22 (41.7 s) and U23 (42.2 s) are slow but make almost no corrections (1.6 / 2.4 deletes). U12 (28.9 s) is medium-fast but makes 21 deletes per trial — a different recovery tendency, which the DTW grouping later helps organize.
...
Stability is measured as the **per-position incremental time** (Δ stability = `time_to_stability_ms.diff()` within each prompt curve), which captures the local cost of each position rather than cumulative elapsed time since the start of the prompt.

| contrast | median Δ stability | wrong-first rate |
|---|---:|---:|
| no layout-switch | 0.28 s | 10.8 % |
| **layout-switch required** | **1.15 s** | **33.3 %** |
| no shift | 0.27 s | 10.7 % |
| shift required | 0.68 s | 15.0 % |
| center zone (home row) | 0.24 s | 9.4 % |
| right zone | 0.27 s | 11.3 % |
| left zone | 0.39 s | 11.8 % |

**Interpretation**
- **Layout-switch positions** are the clearest pathology on both dimensions: they are **4× slower** per-position (1.15 s vs 0.28 s) *and* **3× more likely to result in a wrong first keystroke** (33 % vs 11 %). This is consistent with layout-confusion — the participant must switch between the Thai and English Kedmanee layers and often commits to the wrong key. The previous cumulative-time metric masked this slowdown because layout-switch keys tend to appear mid-prompt, where cumulative time is already large regardless of per-key difficulty.
- **Shift positions** show a clear penalty in both dimensions: 0.68 s vs 0.27 s per-position delta, and 15 % vs 10.7 % wrong-first rate. The shift mechanism adds a reliable but smaller burden compared with layout switching.
- **Left zone is slowest** (0.39 s per-position delta), consistent with the Thai layout concentrating several tone marks and diacritics on the left hand's weaker fingers.
...
The stacked bar chart shows that **dominant recovery tendencies are visible across prompts**. U04 (fast) and U26 (slowest) are very different on speed but both show a substantial share of typing-through-before-backing-up episodes. Meanwhile U05, U15, U22, U23 have very few episodes at all and almost none deep — a more careful, stop-on-error tendency.
...
Average-linkage hierarchical clustering over DTW distances between 120 rich error-episode windows (3 events before / episode / 3 events after) produces 4 exploratory clusters that broadly align with the heuristic recovery summaries:
...
- **U16 on P14** — taller spikes, larger MP variance, and a clear discord around a named entity; consistent with U16's "deep overshoot" tag.
...
Recovery behavior shows **suggestive user-level structure**, but the cluster evaluation should be read as exploratory support rather than proof of sharply separated styles. The heuristic summaries, DTW grouping, and Stage 5B per-trial MP are mutually consistent in showing recurring recovery shapes within some users' trials. The stronger RQ4 evidence is the combination of recovery-episode summaries, representative MP motifs/discords, and per-user traces; the clusters provide descriptive organization for those patterns.
...
3. **Users differ in *what* they do after an error, independently of how fast they type.** RQ4's exploratory recovery grouping cuts across RQ1's speed ranking — the fastest user (U01) and slowest user (U26) can show similar recovery tendencies, while two similarly-fast users (U11, U13) may differ in their recovery patterns.
